...
| `get_architecture()`      | The repo's global digest                              | `POST /v1/:ns/code/architecture` |
| `get_module(path)`        | A directory's digest                                  | `POST /v1/:ns/code/module` |
...
- **tree-sitter function/type-boundary chunking** (`src/treesit.rs`; Rust, Python,
  JS/JSX, TS, TSX, Go) — falls back to a heuristic line/symbol chunker on unsupported
...
reads chunks, not trees, so the digests only matter once the digest tools
...
Both embeddings and LLM summaries route through the litellm gateway. `GatewayEmbedder`
is the default embedder; an optional `FallbackEmbedder` (`ENGRAM_EMBED_FALLBACK=true`)
wraps it with a local `OllamaEmbedder` and delegates `signature()` to the primary, so a
failover never orphans chunks. The deploy runs `mxbai-embed-large` (dim 1024) for
embeddings and `deepseek-chat` for summaries.
...
an accepted trade. Details and the full phase history are in `eval/RESULTS.md`.
